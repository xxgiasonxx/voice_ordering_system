...
def create_menu_database(excel_file='morning_eat.xlsx'):
    ...
    # 插入資料
    # 主餐
    menu_df = xl.parse('food_item', usecols=['id', 'class', 'name', 'price', 'double_egg', 'cheese', 'kimchi', '山丹型麥', 'combo', '素食', '推薦'])
    menu_df['double_egg'] = menu_df['double_egg'].apply(lambda x: 1 if x == '有' else 0)
    menu_df['cheese'] = menu_df['cheese'].apply(lambda x: 1 if x == '有' else 0)
    menu_df['kimchi'] = menu_df['kimchi'].apply(lambda x: 1 if x == '有' else 0)
    menu_df['danish'] = menu_df['山丹型麥'].apply(lambda x: 1 if x == '有' else 0)
    menu_df['combo'] = menu_df['combo'].apply(lambda x: "A/B/C/D" if x == 'A/B/C/D' else 0)
    ...
    final_column_names = ['id', 'class', 'name', 'price', 'double_egg', 'cheese', 'kimchi', 'danish', 'combo', 'vegetarian', 'recommended']
