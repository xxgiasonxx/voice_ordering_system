...
import torch
import re
from CRUD_database import create_connection, query_main_menu, query_combo_menu
...
def load_menu_to_vectorstore(persist_directory="./chroma_db"):
    try:
        vectorstore = Chroma(
            collection_name="morning_kitchen_menu",
...
# # 從 LLM 回應解析 id 編號及數量
# def parse_llm_response(response, query, docs, order_state):
#     updated = False
#     new_order_state = order_state.copy()

#     # 檢查結束或確認
#     if query.lower() in ["結束", "確認", "結帳"]:
#         if new_order_state["items"]:
#             new_order_state["status"] = "confirm"
#         else:
#             new_order_state["status"] = "end"
#         updated = True
#         return new_order_state, updated
#     elif query.lower() in ["內用", "外帶"] and new_order_state["status"] == "confirm":
#         new_order_state["status"] = "end"
#         updated = True
#         return new_order_state, updated

#     # 提取系統部分的 id 和數量（格式：+ ID 數量 或 - ID 數量）
#     id_pattern = r"([+-])\s*(\w+)\s*(\d+)"
#     matches = re.findall(id_pattern, response)

#     if not matches and "無對應品項" not in response:
#         return new_order_state, updated

#     for match in matches:
#         action, item_id, quantity = match
#         quantity = int(quantity)
#         item_id = item_id.upper() if item_id.startswith(('A', 'B', 'C', 'D')) else item_id

#         # 從 RAG 檢索查找品項
#         matched_doc = None
#         for doc in docs:
#             if doc.metadata.get("id") == item_id:
#                 matched_doc = doc
#                 break
...
#         if action == "-":
#             for item in new_order_state["items"][:]:
#                 if item["id"] == item_id:
#                     if item["quantity"] <= quantity:
#                         new_order_state["items"].remove(item)
#                         new_order_state["total_price"] -= item["price"] * item["quantity"]
#                     else:
#                         item["quantity"] -= quantity
#                         new_order_state["total_price"] -= item["price"] * quantity
#                     updated = True
#             continue

#         # 提取客製
#         custom_pattern = r"(雙蛋|起司|泡菜|無糖|去冰|少冰|加糖|[中大]杯)"
#         customizations = re.findall(custom_pattern, query.lower())
#         price = matched_doc.metadata["price"]
#         if "大冰紅" in query.lower() and matched_doc.metadata["name"] == "古早紅茶":
#             customizations.append("大杯")
#             price = 25  # 大杯紅茶 25 元
#             item_id = "112"  # 假設 id 112 為大杯紅茶

#         # 套餐檢查
#         combo_match = re.search(r"\b(a|b|c|d)\b", query.lower())
#         if combo_match and not item_id.startswith(('A', 'B', 'C', 'D')):
#             combo_type = combo_match.group(1).upper()
#             combo_prices = {"A": 35, "B": 50, "C": 50, "D": 50}
#             combo_desc = {
#                 "A": "薯餅+中杯古早紅茶",
#                 "B": "薯條+中杯古早紅茶",
#                 "C": "麥克雞塊+中杯古早紅茶",
...
#             }
#             item_info = {
#                 "id": f"{combo_type}{item_id}",
#                 "name": f"{matched_doc.metadata['name']} {combo_type}套餐",
#                 "price": price + combo_prices[combo_type],
#                 "type": "套餐",
#                 "quantity": quantity,
#                 "components": {"main": matched_doc.metadata["name"], "combo": combo_desc[combo_type]},
#                 "customizations": customizations
#             }
#         else:
#             item_info = {
#                 "id": item_id,
#                 "name": matched_doc.metadata["name"],
#                 "price": price,
#                 "type": matched_doc.metadata["class"],
#                 "quantity": quantity,
#                 "customizations": customizations
#             }
...
#         # 更新或新增品項
#         for item in new_order_state["items"]:
#             if item["id"] == item_id and item["customizations"] == customizations:
#                 item["quantity"] += quantity
#                 new_order_state["total_price"] += item_info["price"] * quantity
#                 updated = True
#                 break
#         else:
#             new_order_state["items"].append(item_info)
#             new_order_state["total_price"] += item_info["price"] * quantity
#             updated = True
...
def rag_query(query, vectorstore, order_state):
    ...
    system_prompt = '''
      ...
      **指令**：
      ...
      - **格式**：
        ...
        - 取消：`- <id> <數量> <客製化>`（例如「- 1 1 無」表示取消 id 1 品項 1 份）。
        - 每行一個動作，無動作時留空。
      - **意圖**：
        - **點餐 (order)**：提取品項 id 和數量（「我要一份蛋餅」→ 「+ 1 1 無」）。
        - **取消 (cancel)**：識別取消品項（「蛋餅不要了」→ 「- 1 1 無」）。
      ...
      - **點餐**：列出品項名稱、價格、客製，詢問是否繼續點餐或客製（例如「好的，原味蛋餅 30 元！要不要加起司或泡菜或套餐？」）。
      ...
      {
          "items": [
              {"id": "1", "name": "原味蛋餅", "price": 30, "type": "台式蛋餅", "quantity": 1, "customizations": []},
              {"id": "112", "name": "古早紅茶", "price": 25, "type": "特調飲品", "quantity": 1, "customizations": ["大杯"]}
          ],
          "total_price": 55,
          "status": "ongoing"
      }
      ...
      5. **查詢**：有什麼飲料？
        ...
        ```
        ```cus
        我們有古早紅茶 20 元、英式奶茶 25 元、鮮奶咖啡 40 元，你想喝啥？
        ```
        ```
    ...
    # 用 Ollama 生成回應
    try:
        response = ollama.generate(model="qwen:0.6b", prompt=prompt, options={
            "temperature": 0.7,
            "top_k": 30,
            "top_p": 0.9,
            "num_ctx": 2048
        ...
        # 解析 LLM 回應，更新訂單
        new_order_state, updated = parse_llm_response(response, query, docs, order_state)
        # 提取顧客回應（移除系統部分）
        customer_response = re.sub(r"```sys\n.*?\n```", "", response, flags=re.DOTALL).strip()
        customer_response = re.sub(r"```cus\n(.*?)```", r"\1", customer_response, flags=re.DOTALL).strip()
...
def change_order(order_state, status, item, quantity, customizations):
    if status == "+":
...
def parse_llm_response(response, order_state):
    # 找到sys和cus的區塊
